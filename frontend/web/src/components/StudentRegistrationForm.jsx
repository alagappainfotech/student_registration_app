...
export default function StudentRegistrationForm({ token, navigate, studentId }) {
  ...
  const fetchClasses = useCallback(async (organizationId) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/v1/classes?organization_id=${organizationId}`, {
  ...
  const fetchSections = useCallback(async (classId) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/v1/sections?class_id=${classId}`, {
        headers: { Authorization: `Bearer ${token}` },
        signal
      });
  ...
  const fetchCourses = useCallback(async (sectionId) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/v1/courses?section_id=${sectionId}`, {
        headers: { Authorization: `Bearer ${token}` },
        signal
      });
